...
This tutorial cover the basis of the integration of a new sensor in _ROS2_ environment.
...
2. Still with librealsense, the script wait for incomming data and get them:
...
3. Then, the reminder of the script consists in converting and displaying the data based on _Numpy_ and _OpenCV_
...
Try to limit the `pipeline.wait_for_frames()` with a very small duration in order to break the infinit loop with a ros timer with call back function.
