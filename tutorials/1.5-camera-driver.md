...
- We want to keep the control on the infinite loop. That for we cannot use ros `spin` function anymore, and we will prefer `spin_once`. To notice that `spin_once` block until an event occurs. It is possible to overpass that by specifying a timeout (`spin_once(myNode, timeout_sec=0.01)`).
